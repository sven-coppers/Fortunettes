...
WXRController.prototype.extendPrototype({
   ...
   handleEvent: function (identifier, event, newWindowState, isPreview) {
        ...
        if(identifier === "low") {
            newWindowState.getWidgetState("tilt").value = 25;
        } else if(identifier === "mid") {
            newWindowState.getWidgetState("tilt").value = 30;
        } else if(identifier === "high") {
            newWindowState.getWidgetState("tilt").value = 35;
        }

        if(!isPreview && newWindowState.getWidgetState("tilt").value == 25) {
            logger.taskReached();
        }
    }
});